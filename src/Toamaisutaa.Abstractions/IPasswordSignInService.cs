...
public interface IPasswordSignInService
{
    ...
    /// <summary>
    /// Finishes a sign-in that stopped at <see cref="SignInOutcome.TwoFactorRequired"/>. The
    /// <paramref name="code"/> is a TOTP code or a recovery code; the caller does not have to say
    /// which.
    /// </summary>
    Task<SignInResult> VerifyTwoFactorAsync(string challengeToken, string code, CancellationToken cancellationToken = default);
}
